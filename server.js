...
function cleanup() {
  ...
  return { students: oldStudents.changes, registrations: oldRegistrations.changes };
}
...
app.get('/api/admin/dashboard', requireAuth, requireAdmin, (req, res) => {
  ...
  const sections = CLASSES.map(className => ({
    className,
    registration: registrationMap.get(className) || null,
    studentCount: students.filter(s => s.class_name === className).length
  }));

  res.json({ stats, registeredClasses: registrations.length, totalClasses: CLASSES.length, sections });
});
...
app.post('/api/db/:number/unlock', (req, res) => {
  ...
  if (![1, 2, 3, 4].includes(number)) return publicError(res, 404, 'Database not found.');
...
app.get('/api/audit', requireAuth, requireAdmin, (req, res) => res.json({ audit: db.prepare('SELECT * FROM audit_log ORDER BY timestamp DESC LIMIT 250').all() }));
